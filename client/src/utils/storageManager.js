...
class StorageManager {  constructor() {
  ...
  addWatermark(suggestions, userTier) {
    if (userTier === 'free') {
      return `${suggestions}\n\n---\n🏷️ Generated by MediScan Free - Sign up for ad-free results and permanent storage!`;
    }
    return suggestions;
  }

  // Store barcode scan data with all extracted information
  async storeBarcodeData(userId, barcodeData, suggestions, userTier) {
    try {
      if (!userId) {
        console.log('No user ID - storing barcode data locally for free tier');
        const localScans = JSON.parse(localStorage.getItem('freeTierBarcodeScans') || '[]');
        localScans.push({
          ...barcodeData,
          suggestions,
        ...
        // Keep only last 5 scans for free tier
        if (localScans.length > 5) {
          localScans.splice(0, localScans.length - 5);
        }
        
        localStorage.setItem('freeTierBarcodeScans', JSON.stringify(localScans));
        return { success: true, storage: 'local' };
      }

      // Store in Firestore for registered users
      const scanRef = doc(db, `users/${userId}/scans`, `scan_${Date.now()}`);
      await setDoc(scanRef, {
        ...
        userTier,
        timestamp: new Date(),
        medicationName: barcodeData.medicationName || 'Unknown',
        ...
        manufacturer: barcodeData.manufacturer,
        dosage: barcodeData.dosage
      });

      console.log('Barcode scan data stored successfully');
  ...
  async getBarcodeScans(userId, userTier) {
    try {
      if (!userId) {
        // Return local storage for free tier
        const localScans = JSON.parse(localStorage.getItem('freeTierBarcodeScans') || '[]');
        return localScans;
      }

      // Get from Firestore for registered users
      const scansRef = collection(db, `users/${userId}/scans`);
      ...
      console.error('Error getting barcode scans:', error);
      return [];
    }
  }
  ...
  async runDailyCleanup() {
    try {
      ...
      console.log(`✅ Daily cleanup complete. Cleaned ${totalCleaned} expired scans.`);
...
export const storeBarcodeData = (userId, barcodeData, suggestions, userTier) => 
  storageManager.storeBarcodeData(userId, barcodeData, suggestions, userTier);

export const getBarcodeScans = (userId, userTier) => 
  storageManager.getBarcodeScans(userId, userTier);
