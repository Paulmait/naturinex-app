-- ============================================================================
-- FIX RLS SECURITY WARNINGS - VERSION 2 (FINAL)
-- ============================================================================
...
-- 4. Using correct column names from actual schema
-- ============================================================================
...
-- user_journeys: Users can manage their own journeys
-- Note: user_journeys uses is_public (not visibility) and status (not is_active)
...
CREATE POLICY "Anyone can view public journey updates"
    ...
    USING (
        EXISTS (
            ...
            AND user_journeys.is_public = TRUE
            AND user_journeys.status IN ('active', 'completed')
        )
    );
...
'Dashboard view for enterprise organizations. Uses security_invoker to run with caller permissions.';
...
GRANT SELECT, INSERT, UPDATE, DELETE ON user_journeys TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON journey_updates TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON remedy_recipes TO authenticated;
...
CREATE INDEX IF NOT EXISTS idx_user_journeys_public ON user_journeys(is_public, status) WHERE is_public = TRUE;
CREATE INDEX IF NOT EXISTS idx_user_journeys_user ON user_journeys(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_journey_updates_user ON journey_updates(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_journey_updates_journey ON journey_updates(journey_id, created_at DESC);
...
BEGIN
    ...
    RAISE NOTICE 'RLS Coverage: %%%', ROUND((enabled_count::NUMERIC / NULLIF(total_tables, 0)) * 100, 2);
    RAISE NOTICE '';
    ...
    ELSE
        RAISE WARNING '⚠ WARNING: % tables are still missing RLS', rls_count;
        RAISE NOTICE 'Note: Some tables may not exist yet in your database.';
    END IF;
    ...
    RAISE NOTICE '- ✅ Used correct column names (is_public, status)';
    RAISE NOTICE '============================================================================';
END $$;
