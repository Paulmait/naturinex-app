...
CREATE INDEX idx_admin_access_logs_admin ON admin_access_logs(admin_id, timestamp DESC);
CREATE INDEX idx_admin_access_logs_ip ON admin_access_logs(ip_address);
...
CREATE INDEX idx_admin_security_alerts_admin ON admin_security_alerts(admin_id, is_read);
...
-- Create trigger on scan_logs to check daily
...
DO $$
BEGIN
  -- Create super admin for guampaul@gmail.com if not exists
  IF NOT EXISTS (SELECT 1 FROM admin_profiles WHERE email = 'guampaul@gmail.com') THEN
    INSERT INTO admin_profiles (
      ...
      (SELECT id FROM auth.users WHERE email = 'guampaul@gmail.com' LIMIT 1),
      'guampaul@gmail.com',
      'owner',
      '{"all": true}'::JSONB,
      true,
...
BEGIN
  ...
  RAISE NOTICE 'Super admin: guampaul@gmail.com';
END $$;