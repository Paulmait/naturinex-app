...
export default function AppLaunchGate({ children }) {
  ...
  const checkLaunchGateStatus = async () => {
    try {
      ...
      if (!ageValid && appConfig.FEATURES.requireAgeVerification) {
        setShowAgeVerification(true);
      } else if (!disclaimerValid && appConfig.FEATURES.showMedicalDisclaimer) {
  ...
  const handleAgeConfirmation = async (isOver17) => {
    ...
    try {
      ...
      if (disclaimerAccepted !== LAUNCH_GATE_VERSION && appConfig.FEATURES.showMedicalDisclaimer) {
        setShowDisclaimer(true);
      } else {
        setHasAcceptedAll(true);
      }
        ...
        <SafeAreaView style={styles.container}>
          <View style={styles.content}>
            ...
            <Text style={styles.legalText}>
              ...
              <Text 
                style={styles.link} 
                onPress={() => openLink(appConfig.APP_CONFIG.termsOfServiceUrl)}
              >
                Terms of Service
              </Text>
              {' '}and{' '}
              <Text 
                style={styles.link} 
                onPress={() => openLink(appConfig.APP_CONFIG.privacyPolicyUrl)}
              >
                Privacy Policy
              </Text>
            </Text>
